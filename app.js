...
function normalizeAuthorName(name) {
  return ADMIN_NAME_OPTIONS.includes(name) ? name : ADMIN_NAME_OPTIONS[0];
}

function populateAuthorOptions() {
  senderNameInput.innerHTML = "";

  ADMIN_NAME_OPTIONS.forEach((name) => {
    const option = document.createElement("option");
    option.value = name;
    option.textContent = name;
    senderNameInput.append(option);
  });
}
...
function loadAuthorName() {
  ...
  const next = normalizeAuthorName(saved || "");
  window.localStorage.setItem(PROFILE_NAME_KEY, next);
  return next;
}

function saveAuthorName(name) {
  const next = normalizeAuthorName(name);
  state.authorName = next;
  window.localStorage.setItem(PROFILE_NAME_KEY, next);
}
...
function getDraftAuthorName() {
  return normalizeAuthorName(senderNameInput.value);
}
...
function beginEditingMessage(message) {
  ...
  senderNameInput.value = normalizeAuthorName(message.authorName || state.authorName);
...
async function sendMessage() {
  ...
  const customName = normalizeAuthorName(senderNameInput.value);
  if (!customName) {
