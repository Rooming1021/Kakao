...
const MAX_IMAGE_DATA_URL_LENGTH = 2400000;
const INITIAL_IMAGE_QUALITY = 0.86;
...
function updateImagePreview() {
  const hasImage = Boolean(state.pendingImageDataUrl);
...
function applyComposerState() {
  ...
  removeImageButton.disabled = disabled || !state.pendingImageDataUrl;
...
function beginEditingMessage(message) {
  ...
  if (message.imageUrl) {
    setPendingImage(message.imageUrl, "기존 이미지");
  } else {
    clearPendingImage();
  }
...
async function fetchMessages({ silent = false } = {}) {
  ...
  try {
    const params = new URLSearchParams({
      select: "id,author_id,author_name,body,image_data_url,created_at",
    ...
    console.error(error);
    state.lastError = "메시지를 불러오지 못했습니다. Supabase 정책과 SQL 컬럼 설정을 확인하세요.";
...
async function handleImageSelection(event) {
  ...
  try {
    ...
    setPendingImage(dataUrl, file.name || "새 이미지");
